...
<div class="pagingGrp">
	<a href="Reg.do?bid=${param.bid}"  class="dBtn3">글쓰기</a>
	<a href="Reply.do?bid=${param.bid}&GROUPID=${detail.GROUPID}&DEPTH=${detail.DEPTH}&ORDERINDEX=${detail.ORDERINDEX}"  class="dBtn3">답글쓰기</a>
	
	<c:if test="${username == detail.WRITER || adminRole == true}">		
		<a href="getUpdate.do?${linkURL}" class="dBtn3" >수정하기</a>
	</c:if>
	<%-- </c:if> --%>
	<a href="List.do?${linkURL}"  class="dBtn3">목록보기</a>
	
	<c:if test="${username == detail.WRITER || adminRole == true}">		
		<a href="javascript:fn_BoardDelete('${linkURL}&GROUPID=${detail.GROUPID}&DEPTH=${detail.DEPTH}&ORDERINDEX=${detail.ORDERINDEX}')" class="dBtn5">삭제하기</a>
	</c:if>
</div>
...
<div class="pagingGrp">
	<a href="Detail.do?currentPageNo=${pageConfig.currentPageNo - 10}&${linkURL}">이전</a>
	<c:forEach var="i" begin="${pageConfig.startPage}" end="${pageConfig.endPage}">
		<c:if test="${ i == pageConfig.currentPageNo }">
			<strong>${i}</strong>
		</c:if>
		<c:if test="${ i != pageConfig.currentPageNo }">
			<a href="Detail.do?currentPageNo=${i}&${linkURL}">${i}</a>
		</c:if>
	</c:forEach>
	<a href="Detail.do?currentPageNo=${pageConfig.currentPageNo + 10}&${linkURL}">다음</a>
</div> 