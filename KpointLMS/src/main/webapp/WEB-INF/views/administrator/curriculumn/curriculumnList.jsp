...
<c:url value="bid=${param.bid}&CURSTATUS=${param.CURSTATUS}&KEYWORD=${param.KEYWORD}&KEYSTRING=${param.KEYSTRING}" var="linkURL" />
<script>
	console.log("${param}");
	var linkURL = "${linkURL}";
	console.log("!!!!!!!!!!!!!!!!!!!!" + linkURL);
...
<c:out value="${param.CURSTATUS}" />
<div class="searchBox">
	<form id="curriculumnSearchForm" action=curriculumnList.do name="curriculumnSearchForm" method="GET">
	...
	<div class="searchBtnGrp">
		<a href="" class="confirm" id="curriculumnListSearchBtn">검색</a>
	</div>
	</form>
</div>
<div class="tableBox">
<table class="listTbl">
	...
	<c:if test="${clist.size() != 0 }">
		<c:forEach var="cList" items="${clist}" varStatus ="st">
			<tr>
				...
				<td class="left"><a href="curriculumnDetail.do?SEQ=${cList.SEQ}&${linkURL}&currentPageNo=${pageConfig.currentPageNo}">${cList.CURPRONAME}</a></td>
				<td>
	...
	<c:if test="${clist.size() == 0 && CURSTATUS eq 'start'}">
		<td colspan="13">진행중인 강좌가 없습니다.</td>
	</c:if>
	<c:if test="${clist.size() == 0 && CURSTATUS eq 'wait'}">
		<td colspan="13">개설중인 강좌가 없습니다.</td>
	</c:if>
	<c:if test="${clist.size() == 0 && CURSTATUS eq 'finish'}">
		<td colspan="13">종료된 강좌가 없습니다.</td>
	</c:if>
</table>
</div>
<div class="pagingGrp">
	<a href="curriculumnRegister.do?${linkURL}&currentPageNo=${pageConfig.currentPageNo}" class="dBtn3 floatRight">새 교육과정 등록</a>
	<a href="curriculumnList.do?currentPageNo=${pageConfig.currentPageNo - 10}&${linkURL}">이전</a>
	<c:forEach var="i" begin="${pageConfig.startPage}" end="${pageConfig.endPage}">
		<c:if test="${ i == pageConfig.currentPageNo }">
			<strong>${i}</strong>
		</c:if>
		<c:if test="${ i != pageConfig.currentPageNo }">
			<a href="curriculumnList.do?currentPageNo=${i}&${linkURL}">${i}</a>
		</c:if>
	</c:forEach>
	<a href="curriculumnList.do?currentPageNo=${pageConfig.currentPageNo + 10}&${linkURL}">다음</a>
</div> 
