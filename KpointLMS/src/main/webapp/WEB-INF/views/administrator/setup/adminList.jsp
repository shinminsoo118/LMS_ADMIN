...
<div class="searchBox">
	<form id="adminSearchForm" action=adminList.do name="adminSearchForm" method="GET">
	...
	<div class="searchBtnGrp">
		<a href="" class="confirm" id="adminListSearchBtn">검색</a>
	</div>
	</form>
</div>
<div class="tableBox">
<table class="listTbl">
	...
	<c:forEach items="${userList}" var="list" varStatus="status">
		<tr>
			...
			<td>${list.COUNT}</td>
			<td id="userId"><a href="adminDetail.do?USERID=${list.USERID }">${list.USERID}</a></td>
			<td>${list.USERNAME}</td>
...
<div class="pagingGrp">
	<a href="./adminRegister.do?bid=${param.bid}&currentPageNo=${pageConfig.currentPageNo}&${linkURL}" class="dBtn3 floatRight">관리자등록</a>
	<a href="adminList.do?currentPageNo=${pageConfig.currentPageNo - 10}&${linkURL}">이전</a>
	<c:forEach var="i" begin="${pageConfig.startPage}" end="${pageConfig.endPage}">
		<c:if test="${ i == pageConfig.currentPageNo }">
			<strong>${i}</strong>
		</c:if>
		<c:if test="${ i != pageConfig.currentPageNo }">
			<a href="adminList.do?currentPageNo=${i}&${linkURL}">${i}</a>
		</c:if>
	</c:forEach>
	<a href="adminList.do?currentPageNo=${pageConfig.currentPageNo + 10}&${linkURL}">다음</a>
</div>
