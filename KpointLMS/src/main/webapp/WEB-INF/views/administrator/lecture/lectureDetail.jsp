...
<c:if test="${param.dStatus == 1 }" >
	<script>
		alert("수강을 취소하였습니다.");
		location.href = "lectureList.do?${linkURL}"
	</script>
</c:if>
<c:if test="${param.dStatus == 0 }" >
	<script>
		alert("수강취소를 실패하였습니다.");
	</script>
</c:if>
...
<div class="tableBox">
	...
	<div class="btnGrp">
		<a href="lecturePayment.do?${linkURL}&currentPageNo=${param.currentPageNo}" class="confirm">결제하기</a>
		<a href="lectureSignUpDelete.do?${linkURL}&currentPageNo=${param.currentPageNo}" class="confirm">수강취소</a>
		<a href="lectureList.do?${linkURL}&currentPageNo=${param.currentPageNo}" class="cancel">취소</a>
	</div>
