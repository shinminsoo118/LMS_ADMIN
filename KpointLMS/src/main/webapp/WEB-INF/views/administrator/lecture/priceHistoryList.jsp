	...
	<c:if test="${clist.size() != 0 }">
		<c:forEach var="cList" items="${clist}" varStatus ="st">
			<tr>
				...
				<td class="left"><a href="priceHistoryDetail.do?SEQ=${cList.PSEQ}&${linkURL}&currentPageNo=${pageConfig.currentPageNo}">${cList.CURPRONAME}</a></td>
				<td>
...
<div class="pagingGrp">
	<a href="priceHistoryList.do?currentPageNo=${pageConfig.currentPageNo - 10}&${linkURL}">이전</a>
	<c:forEach var="i" begin="${pageConfig.startPage}" end="${pageConfig.endPage}">
		<c:if test="${ i == pageConfig.currentPageNo }">
			<strong>${i}</strong>
		</c:if>
		<c:if test="${ i != pageConfig.currentPageNo }">
			<a href="priceHistoryList.do?currentPageNo=${i}&${linkURL}">${i}</a>
		</c:if>
	</c:forEach>
	<a href="priceHistoryList.do?currentPageNo=${pageConfig.currentPageNo + 10}&${linkURL}">다음</a>
</div>
