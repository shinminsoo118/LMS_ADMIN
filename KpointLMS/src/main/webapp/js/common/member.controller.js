...
//		$(".popWrap").center();
	...
	$("#toRemove").click(function(){
		console.log($("#selectBox option:selected").val());
		if($("#selectBox option:selected").val() == undefined){
			alert("삭제할 대상을 선택하세요.!")
		}else{
			$("#selectBox option:selected").remove();
		}
	...
	$("#selectBoxCC").dblclick(function(){
		console.log($("#selectBoxCC option:selected").val());
		$("#selectBoxCC option:selected").remove();
		//		console.log($("#selectBox option:selected"));
	});
	
	$("#ccRemove").click(function(){
		console.log($("#selectBoxCC option:selected").val());
		if($("#selectBoxCC option:selected").val() == undefined){
			alert("삭제할 대상을 선택하세요.!")
		}else{
			$("#selectBoxCC option:selected").remove();
		}
	...
	$("#addressPopup").click(function(){
		...
		$("#selectBoxCC option").remove();
		showPopup();
		
		var formName = "regForm";
		var url = "userMailList.do";
		var dType = "json";
		fn_postAjax(formName, url, dType, fn_addressUserSuccess);
	});
	...
	$("#smsPopup").click(function(){
		...
		var formName = "regForm";
		var url = "userSmsList.do";
		var dType = "json";
		fn_postAjax(formName, url, dType, fn_smsUserSuccess);
	});
	
	function fn_addressUserSuccess(data){
		console.log("data", data);
		var str = "";
		for(var i = 0; i < data.length; i++){
			str +='<input name="checkbox" type="checkbox" value="'+ data[i].USEREMAIL +'">'+ data[i].USERID+'<"'+data[i].USEREMAIL+'"><br>';
		}
		
		$(".checkScroll").html(str);
	}
	
	function fn_smsUserSuccess(data){
		console.log("data", data);
		var str = "";
		for(var i = 0; i < data.length; i++){
			str +='<input name="checkbox" type="checkbox" value="'+ data[i].USERTEL +'">'+ data[i].USERID+'<"'+data[i].USERTEL+'"><br>';
		}
	...
	$("#addGroup").on("click", function(){
		...
		var dType = "json";
		fn_postAjax(formName, url, dType, fn_addressGroupSuccess);
	});
	
	function fn_addressGroupSuccess(data){
		console.log("data", data);
		var str = "";
		for(var i = 0; i < data.length; i++){
			str +='<input name="checkbox" type="checkbox" value="'+ data[i].USERROLE +'">'+ data[i].USERROLE+'<br>';
		}
	...
	// 팝업창 닫기
	$("#addressPopupClose").click(function(e){
		e.preventDefault();
		showPopupClosed();
	});
	...
	$("#addInput").on("click",function(e){
		e.preventDefault();
		var selectSize   = $("#selectBox option").size();
		var selectCCSize = $("#selectBoxCC option").size();
		console.log(selectSize);
		console.log(selectCCSize);
		
		if(selectSize > 0){
			var toStr = "";
			for(var i = 0; i < selectSize; i++){
				toStr += $("#selectBox option:eq('"+ i + "')").val() + ",";
			}
		...
		if(selectCCSize > 0){
			var ccStr = "";
			for(var i = 0; i < selectCCSize; i++){
				ccStr += $("#selectBoxCC option:eq('"+ i + "')").val() + ",";
			}
	...
	//전채선택
	$("#checkboxTrue").click(function(){
		$("input[name=checkbox]").prop("checked", true);
	});
	
	//전체해제
	$("#checkboxFalse").click(function(){
		$("input[name=checkbox]").prop("checked", false);
	});
	
	
    $("#smsSendBtn").on("click", function(e){
    	e.preventDefault();
    	
    	var result = 0;
		
		$(".inputTbl .tblInput").each(function( key,i) {
			var validCheck = 1;
...
function selectChkBox(frm, add){
	var sum = 0;
	var count = frm.checkbox.length;
	
	for(var i=0; i < count; i++){
		if(frm.checkbox[i].checked == true){
			addSelectBox(frm.checkbox[i].value, frm.checkbox[i].value, frm, add);
		}
	}
}

function addSelectBox(text,value,frm, add){
	var isIE = false;
	  //브라우저가 IE인지 체크한다
	...
	if(add == 'to'){
		console.log(add);
		if(isIE){
			frm.selectBox.add(new Option(text,value)); 
		}else{
	    	frm.selectBox.add(new Option(text,value),null,null,null); 
	    } 
	}else{
		console.log(add);
		if(isIE){
			frm.selectBoxCC.add(new Option(text,value)); 
		}else{
	    	frm.selectBoxCC.add(new Option(text,value),null,null,null); 
	    } 
	}
