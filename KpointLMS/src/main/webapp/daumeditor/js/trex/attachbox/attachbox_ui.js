...
Trex.I.AttachBox = {
	...
	onAttachBoxInitialized: function(config, canvas) {
		...
		this.observeJob(Trex.Ev.__ENTRYBOX_ENTRY_REMOVED, function(entry) {
			/*console.log(entry);
			console.log("이곳?", entry.data.fileStrName)*/
			//하나의 파일을 삭제할 경우 .
			fn_deleteAttachFile('One', entry.data.fileStrName);
