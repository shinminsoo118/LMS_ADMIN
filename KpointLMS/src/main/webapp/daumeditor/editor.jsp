...
<script src="${pageContext.request.contextPath}/daumeditor/js/editor_loader.js?environment=production" type="text/javascript" charset="utf-8" ></script>
<script>
...
<script type="text/javascript">
	...
	var config = {
		...
		txPath: '${pageContext.request.contextPath}/daumeditor/', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) /xxx/xxx/ */
...
<script type="text/javascript">
	...
	function validForm(editor) {
		...
		//커리큘럼 일경우 Validation 체크
		if(OpennerValues.bid == "Curriculumn"){
			if(fn_curriculumnValidate() == false){
				return false;
			}	
		}
		...
		if($("#TITLE").val() == ""){
			alert("제목을 입력하세요.");
			$("#TITLE").focus();
			check_confirm_write = false;
			return false;
		}
		
		if (!validator.exists(content)) {
			alert('내용을 입력하세요');
			$(".tx-content-container").focus();
			Editor.focus();
			check_confirm_write = false;
			return false;
		}
		 
		check_confirm_write = true;
		return true;
	}
	...
	function setForm(editor) {
	    console.log("check_confirm_write", check_confirm_write);
        var i, input;
       ...
       /*  var textarea = document.createElement('textarea');
        ...
        //console.log(fn_insertDom());
        //return false;
        if(OpennerValues.bid == "Curriculumn"){
        	$("#"+formName).append(fn_insertDom());	
        }
        
        //////////////// 첨부파일이 있는지 확인 /////////
        var attachFile = Editor.getAttachments();
        if(attachFile.length > 0){
        	
			//업로드할 파일이 있다면, 파일의 BOARDSEQ값을 UPDATE 함.
			//var fileWriter = attachFile[0].data.fileWriter;
			var fileWriter = OpennerValues.WRITER;
			//var url = "/common/FileUpdate.do?FILEWRITER="+fileWriter;
			var Str = "<input type='hidden' name='TempFILEWRITER' value='"+fileWriter+"' />";
			console.log("------------------@" +Str);
			$("#"+formName).append(Str);
		}
...
<script type="text/javascript">
	function loadContent(data) {
		...
		var fileName, fileStrName, fileSize, extSort, fileWriter, fileSeq;
		
		data.each(function(item, index){
			fileName    = item.FILENAME;
			fileStrName = item.FILESTRNAME;
			fileSize    = item.FILESIZE;
			fileWriter  = item.FILEWRITER;
			fileSeq     = item.FILESEQ;
			extSort     = fileExt(fileStrName);
			console.log(fileName, fileStrName, fileSize, extSort, fileWriter);
			
			if(extSort != "file"){
				attachments['image'].push({
					'attacher': 'image',
					'data': {
						'imageurl': url +'/' + fileStrName,
						'filename': fileName,
						'filesize': fileSize,
...
<script>
	...
	$(window).on("beforeunload", function(){
		console.log("이미지저장하지 않고 나가기");
		var attachFile = Editor.getAttachments();
		
		if(check_confirm_write == false){
			//페이지 내용을 저장하지 않고 그냥 나갈경우..
			//파일을 삭제
			if(attachFile.length > 0){
				/* var fileWriter = attachFile[0].data.fileWriter; */
				var fileWriter = OpennerValues.WRITER;
				console.log("파일작성자 = ",fileWriter);
				var url = "${contextPath}common/FileDelete.do?bid="+OpennerValues.bid+"&TempFILEWRITER="+fileWriter;
				fn_getAjax(url, 'text', deleteFile);
				//return false;
			}
		}
		
		//return "페이지를 벗어나면 내용이 사라집니다.";
	});
...
<script>
	var OpennerValues = {
		bid: '${param.bid}',
		SEQ: '${param.SEQ}',
		imageCount : '${boardConfig.IMAGECOUNT}',
		fileCount : '${boardConfig.FILECOUNT}',
		WRITER: fn_random(0,1000000000)+'-${username}'
	}
	
	console.log("파일카운트 = ",OpennerValues.imageCount, OpennerValues.fileCount);
	//수정시 파일 항목을 가져옴.
	function getEditFile(){
		var url = "${contextPath}/common/FileList.do?bid="+OpennerValues.bid+"&SEQ="+OpennerValues.SEQ;
		var dType = "json";
		fn_getAjax(url, dType, settingFileList);
	}
	
	function settingFileList(data){
		console.log(data);
		loadContent(data);
	}
	...
	function fn_deleteAttachFile(type, fileStrName){
		//다음에디터는 전체파일 삭제를 하면
		//전체목록이 넘어가서 삭제되는것이 아니고, 파일 하나씩 넘어가서 삭제됨.
		//console.log("파일 지우기");
		//var Obj = [];
		var data;
		var fileName;
		
		var fileStrName = fileStrName
		//console.log("fileName = ",fileStrName);
		data = {'fileName': fileStrName, 'type': type, 'bid': OpennerValues.bid};
		
		var url = "${contextPath}/common/FileListDelete.do";
		var dType = "text";
		fn_getAjax(url, dType, fileDelete, data);
	}
